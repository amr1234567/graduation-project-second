...
export class UserViewHeaderComponent implements OnInit{
  ...
  private updateNavigationLinks(categories: CategoryModel[]) {
    const menCategory = categories.find(c => c.name.toLowerCase() === 'men');
    const womenCategory = categories.find(c => c.name.toLowerCase() === 'women');

    this.navigationLinks.update(links => {
      return links.map(link => {
        if (link.value === 'Men' && menCategory) {
          return { ...link, categoryId: menCategory.id };
        }
        if (link.value === 'Women' && womenCategory) {
          return { ...link, categoryId: womenCategory.id };
        }
        return link;
      });
    });
  ...
  private updateActiveLink(query: string): void {
    ...
    this.navigationLinks.update(links => {
      return links.map(link => ({
        ...link,
        isActive: link.categoryId == categoryId
      }));
    });
    ...
    this.personalIcons.update(links => {
      return links.map(link => ({
        ...link,
        isActive: link.route ? query.startsWith(link.route) : false
      }));
    });
  }

  navigateToCategory(link: NavIconType) {
    if (link.categoryId) {
      this.router.navigate([link.route], {
        queryParams: { category: link.categoryId }
      });
    } else {
      this.router.navigate([link.route]);
    }
  ...
  search(value: string) {
    this.router.navigate([], {
      ...
      queryParamsHandling: 'merge', // يحافظ على باقي الباراميترات
    });
  }
